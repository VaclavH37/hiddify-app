...
/// Re-verifies any active Google Play subscription once at launch.
///
/// This is the acknowledgement safety net + restore path from
/// IAP-CLIENT-INTEGRATION.md §5.5: an interrupted verify (app killed / network
/// drop right after purchase), a reinstall, or a sign-in on a new device leaves
/// an active Play purchase that the backend hasn't bound yet. Re-verifying is
/// idempotent server-side and, with the backend returning the cryptolink inline
/// on verify, imports the profile without hitting the 15-min reauth gate.
///
/// It self-guards and stays silent:
///  - Android only (no Billing elsewhere).
...
///  - Naturally bounded: at most the (≤1) active SUBS purchase is re-verified.
